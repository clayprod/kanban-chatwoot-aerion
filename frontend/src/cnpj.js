...
export const normalizeCnpj = (value) => String(value ?? '')
  .trim()
  .toUpperCase()
  .replace(CNPJ_SEPARATORS_RE, '');

export const normalizeFiscalIdentifier = (value) => normalizeCnpj(value);

export const isCnpjFormat = (value) => CNPJ_FORMAT_RE.test(normalizeCnpj(value));
export const isCnpjRoot = (value) => CNPJ_ROOT_RE.test(normalizeCnpj(value));

export const splitCnpj = (value) => {
...
export const calculateCnpjCheckDigits = (firstTwelve) => {
...
export const hasValidCnpjCheckDigits = (value) => {
  const cnpj = normalizeCnpj(value);
